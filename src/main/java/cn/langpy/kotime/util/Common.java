...
public class Common {
    ...
    public static InvokedInfo getInvokedInfo(MethodInvocation invocation, MethodNode parent, double runTime) {
        ...
        StringBuilder entiryBuilder=new StringBuilder();
        for (Object param:paramObjects) {
        	if(null!=paramObjects[0].getClass().getAnnotation(Entity.class)){
        		 entiryBuilder.append(paramObjects[0].getClass().getName()+"#");
        	}
        	
		}
       
        current.setEntity(entiryBuilder.toString());
    ...
    public static InvokedInfo getInvokedChildInfo(MethodInvocation invocation, MethodNode node, double runTime) {
        MethodNode current = MethodNodeService.getCurrentMethodNode(invocation, runTime);
        InvokedInfo invokedInfo = new InvokedInfo();
        node.setValue(1.0);
        invokedInfo.setCurrent(node);
        invokedInfo.setParent(current);
        invokedInfo.setNames(invocation.getMethod().getParameters());
        invokedInfo.setValues(invocation.getArguments());
        return invokedInfo;
    }
